...
unsigned int LRU(void) {
	...
	for (unsigned int i = 0; i < ref_size; i++) {
		...
		clock++;//每一个页面读入内存时加入一个全局每次都累加的计数器，这样最小的那个就是最远的 
		//页面不在帧中
		if (!page_table[reference[i]].flag) {
			printf("but not in the frame.\n");
			diseffect++;
...
int main(void) {
	...
	if (choice1 != '3')
		printf("\nreference generate: 1)auto      2)hand\n");
		char choice = getchar();
		while (getchar() != '\n');
		if (choice == '2') {
		reference = (unsigned int *)malloc(ref_size * sizeof(unsigned int));
		printf("enter the %d reference: ", ref_size);
		for (int i = 0; i < ref_size; i++) {
	        scanf("%u", &reference[i]); }}
		else{
			initial_reference();//初始化引用 
		}
	unsigned int diseffect;//缺页 
