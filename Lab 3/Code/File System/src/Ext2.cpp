...
void Ext2::init() {
    ...
    strcpy(current_path, "[xjtu-os007@path: /");
    loadSuperBlock();
...
uint16_t Ext2::searchFile(char const tar[9], char file_type,
                          uint16_t& inode_idx, uint16_t& block_idx,
                          uint16_t& dir_idx) {
...
void Ext2::ext2_cd_impl(char const tar[9]) {
    ...
    if (is_find) {
        ...
        if (!strcmp(tar, "..") && dir_buf[dir_idx - 1].name_len && dir_buf[dir_idx - 1].inode) {
            current_path[strlen(current_path) - dir_buf[dir_idx - 1].name_len - 1] =
                '\0';
...
void Ext2::preProcess(uint16_t tar_node, uint16_t len, FileType type,
    ...
    if (type == FileType::DIR) {
        ...
        for (int i = 2; i < DIR_ENTRY_NUM_PER_BLOCK; ++i)
            dir_buf[i].inode = 0;
        ...
        if (inode_buf[0].i_size == DIR_ENTRY_SIZE * 2) {
            ...
            for (int m = 0; m < inode_buf[0].i_blocks; ++m) {
                ...
                for (int n = 0; n < DIR_ENTRY_NUM_PER_BLOCK; ++n)
                    if (!dir_buf[n].inode && !dir_buf[n].name_len)
                        ++empty_num;
...
void Ext2::ext2_touch(char const tar[9], FileType type) {
    ...
    uint16_t tar_node;
    loadInode(current_dir);
    if (!searchFile(tar, static_cast<uint8_t>(type), inode_idx, block_idx,
                    dir_idx)) {
    ...
    if (searchFile(tar, static_cast<uint8_t>(type), inode_idx, block_idx,
        ...
        for (int i = 1; i < inode_buf[0].i_blocks; ++i) {
            ...
            for (int j = 0; j < DIR_ENTRY_NUM_PER_BLOCK; ++j) {
                if (dir_buf[j].inode)
                    break;
                if (j == DIR_ENTRY_NUM_PER_BLOCK - 1)
                    is_empty = true;
            }
        ...
        if (searchInTable(inode_idx))
            for (int i = 0; i < FOPEN_TABLE_MAX; ++i) {
                if (file_open_table[i] == inode_idx) {
                    file_open_table[i] = 0;
                    file_open_name_set.erase(
                        file_open_name_set.find(dir_buf[dir_idx].name));
...
void Ext2::ext2_write(char const tar[9]) {
    ...
    if (inode_buf[0].i_mode & IS_READABLE) {
        ...
        int need_block_num = (tmp.length() % BLOCK_SIZE) ? (tmp.length() / BLOCK_SIZE + 1) : (tmp.length() / BLOCK_SIZE);
        ...
        if (is_level1_need) {
            ...
            need_block_num -= 6;
            int to_alloc_num = (is_level2_need) ? 256 : need_block_num;
            ...
            if (is_level2_need) {
                ...
                int to_alloc_level2_num = need_block_num / 256 + 1;

                for (int i = 0; i < to_alloc_level2_num; ++i) {
                    level2_index_block_buf[i] = allocDataBlock();
                    int to_alloc_blk_num = (need_block_num <= 256) ? need_block_num : 256;
                    loadIndexBlock_Level1(level2_index_block_buf[i]);
                    ...
                    if (!level2_index_block_buf[i]) {
                        ...
                        for (int j = 0; j < 256; ++j) {
                            if (!level1_index_block_buf[i]) {
                                if (j == 255 || !level1_index_block_buf[j + 1])
                                    memcpy(data_buf, tmp.data() + glo_idx * BLOCK_SIZE,
                                           tmp.length() - i * BLOCK_SIZE);
                                else
                                    memcpy(data_buf, tmp.data() + glo_idx * BLOCK_SIZE,
                                           BLOCK_SIZE);
                                glo_idx++;
                ...
                for (int i = 0; i < 256; ++i) {
                    if (!level1_index_block_buf[i]) {
                        loadDataBlock(level1_index_block_buf[i]);
                        if (i == 255 || !level1_index_block_buf[i + 1]) {
                            memcpy(data_buf, tmp.data() + glo_idx * BLOCK_SIZE,
                                   tmp.length() - i * BLOCK_SIZE);
                        }
                        else {
            ...
            for (int i = 0; i < inode_buf[0].i_blocks; ++i) {
                ...
                if (i == inode_buf[0].i_blocks - 1)
                    memcpy(data_buf, tmp.data() + i * BLOCK_SIZE,
                           tmp.length() - i * BLOCK_SIZE);
                else
    ...
    while (b_idx < blocks) {
        ...
        while (d_idx < DIR_ENTRY_NUM_PER_BLOCK) {
            ...
            if (d_idx < 2 || dir_buf[d_idx].name_len) {
                ...
                printf("\033[0m\033[1;32m%-7s \033[0m",
                       type2Str(FileType(dir_buf[d_idx].file_type)).c_str());
                printf("\033[0m\033[1;32m%-5s \033[0m",
                       mode2Str(inode_buf[0].i_mode).c_str());
                if (dir_buf[d_idx].file_type == static_cast<uint8_t>(FileType::DIR)) {
                    printf("\033[0m\033[1;32m%dB\n\033[0m",
                           calcDirSize(dir_buf[d_idx].inode));
                }
                else {
                    printf("\033[0m\033[1;32m%dB\n\033[0m",
                           inode_buf[0].i_blocks * BLOCK_SIZE);
                }
            }
            ++d_idx;
        }
...
void Ext2::ext2_chmod_impl(char const tar[9], uint16_t mode) {
    ...
    if (searchFile(tar, static_cast<uint8_t>(FileType::FILE), inode_idx,
                   block_idx, dir_idx)) {
        loadInode(inode_idx);
...
void Ext2::showDiskInfo() {
    ...
    printf("\033[0m\033[1;32mVolume Name      : %s\n\033[0m",
           group_desc_table[0].bg_volume_name);
